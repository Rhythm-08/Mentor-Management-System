    ...
    def forget_password(self):
            if  self.cmb_quest.get()=="Select" or self.txt_answer.get()=="" or self.txt_new_pass.get()=="":
                    ...
                    try:
                            ...
                            cur=con.cursor()
                            cur.execute("Select * from registered where email=? and question=? and answer=?",(self.txt_email.get(),self.cmb_quest.get(),self.txt_answer.get()))
                            row=cur.fetchone()
                            if row==None:
                                    messagebox.showerror("Error","Please Select the correct Security / Enter Answer",parent=self.root2)
                            else:
                                    cur.execute("update registered set password=? where email=?",(self.txt_new_pass.get(),self.txt_email.get(),))
                                    con.commit()
                                    con.close()
    ...
    def forget_password_window(self):
         if self.txt_email.get()=="":
                ...
                try:
                        con=sqlite3.connect(database="mentor.db")
                        cur=con.cursor()
                        cur.execute("select * from registered where email=?",(self.txt_email.get(),))
                        row=cur.fetchone()
                        if row==None:
    ...
    def login(self):
            if self.txt_email.get()=="" or self.txt_pass_.get()=="":
              ...
              try:
                      con=sqlite3.connect(database="mentor.db")
                      cur=con.cursor()
                      cur.execute("Select * from registered where email=? and password=?",(self.txt_email.get(),self.txt_pass_.get()))
                      row=cur.fetchone()
                      if row==None:
